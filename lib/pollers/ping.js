...
PingPoller.prototype.poll = function() {
  ...
  var that = this;
  var deadline = ((this.timeout / 1000) - 1);

  this.debug("Pinging " + this.target);

  this.request = exec("ping -c 1 -w " + deadline + " "+this.target, this.execCallback.bind(this))
}
 ...
 * @api   private
 */
PingPoller.prototype.execCallback = function (error, stdout, stderr) {
  if(error === null) {
